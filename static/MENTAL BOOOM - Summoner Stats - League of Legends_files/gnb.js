...
var setImageBanner = function (data) {
  if (data) {
    var header = $("header");
    var lang = header.data("lang").split("_")[0];
...
$(function () {
  ...
  var region = header.data("region").toLowerCase();
  var config = "https://opgg-gnb.akamaized.net/config/lol.config.json";

  if (region) {
    if (region === "kr" || region === "sg" || region === "ph") {
      config = "https://opgg-gnb.akamaized.net/config/lol." + region + ".config.json";
    }
  }
